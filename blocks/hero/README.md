...
Create a two-column table headed **Hero (services)**, with merged heading cells.
It uses the same Content row and CTA formatting as the home variant:

| Label | Content |
| --- | --- |
| Eyebrow | Adobe Services. Optional plain text; displayed as an overline, not a heading. |
| Content | One Heading 1, a normal description paragraph, then CTA links in separate paragraphs. Bold links are primary; italic or unformatted standalone links are secondary. |

Suggested content:

> Six ways we take AEM<br>off your critical path

Use one Heading 1 with a soft line break after “AEM” to match the desktop
composition. Enter the description as one paragraph without forced line breaks:

> Pick by problem, not by package. Every service page states what it covers, who runs it and what changes in your delivery.

Suggested CTAs: **Discuss your AEM challenge** and *Book a free AEM audit*.
Set real destinations in the document; the block does not supply URLs. Text,
formatting, link destinations, and new-tab choices remain authored content.
An empty Eyebrow row is omitted. One or no CTA is supported without an empty
actions container. Services does not require Background, Badges, or Trust rows.

The Services variant has a full-width dark surface, a centered 1200 px content
grid, and left-aligned copy. At 1440 px, content begins 120 px from the edge.
Below 600 px the CTAs stack; from 600 px they share a row when space permits.
The heading scales at 600 and 900 px, and content determines the section height.
The mobile layout is a responsive adaptation; no separate Services mobile design
export was supplied. Breadcrumbs and the following statistics section are separate
content and are not generated by this block.

Keep the Hero in its own EDS section using an unformatted `---` paragraph after
