...
- Pause/Resume animation is a native button supporting Enter/Space and visible
  focus. Hover or focus inside the block also pauses motion; after Resume, motion
  continues when the pointer/focus leaves. A manual pause persists across resizing.
- `prefers-reduced-motion` disables motion and hides the unnecessary pause button,
  including when the setting changes while the page is open. Mobile never animates.
...
Check both animation directions, loop boundaries, hover/focus, keyboard pause,
